 ...
 * @since  1.5
 */
class JMediaControllerFolder extends BaseController
{
	/**
	...
	public function delete()
	{
		...
		if (empty($paths))
		{
			$this->setMessage(\Joomla\CMS\Language\Text::_('JERROR_NO_ITEMS_SELECTED'), 'error');
		...
		if (!$user->authorise('core.delete', 'com_jmedia'))
		{
			...
			JError::raiseWarning(403, \Joomla\CMS\Language\Text::_('JLIB_APPLICATION_ERROR_DELETE_NOT_PERMITTED'));
		...
		foreach ($unsafePaths as $path)
		{
			...
			$app->enqueueMessage(\Joomla\CMS\Language\Text::sprintf('COM_JMEDIA_ERROR_UNABLE_TO_DELETE_FILE_WARNFILENAME', $path), 'error');
		}
		...
		foreach ($safePaths as $path)
		{
			...
			{
				JError::raiseWarning(100, \Joomla\CMS\Language\Text::_('COM_JMEDIA_ERROR_WARNINVALID_FOLDER'));
			...
			if (is_file($object_file->filepath))
			{
				...
				if (in_array(false, $result, true))
				{
					...
					JError::raiseWarning(100, \Joomla\CMS\Language\Text::plural('COM_JMEDIA_ERROR_BEFORE_DELETE', count($errors), implode('<br />', $errors)));
				...
				$app->enqueueMessage(\Joomla\CMS\Language\Text::sprintf('COM_JMEDIA_DELETE_COMPLETE', substr($object_file->filepath, strlen(COM_JMEDIA_BASE))));
			}
			elseif (is_dir($object_file->filepath))
			{
				...
				if (!empty($contents))
				{
					...
					JError::raiseWarning(100, \Joomla\CMS\Language\Text::sprintf('COM_JMEDIA_ERROR_UNABLE_TO_DELETE_FOLDER_NOT_EMPTY', $folderPath));
	...
	public function create()
	{
		...
		if (strlen($folder) > 0)
		{
			if (!$user->authorise('core.create', 'com_jmedia'))
			{
				...
				JError::raiseWarning(403, \Joomla\CMS\Language\Text::_('COM_JMEDIA_ERROR_CREATE_NOT_PERMITTED'));
			...
			{
				$app = JFactory::getApplication();
				$app->enqueueMessage(\Joomla\CMS\Language\Text::_('COM_JMEDIA_ERROR_UNABLE_TO_CREATE_FOLDER_WARNDIRNAME'), 'warning');
			...
			{
				$app = JFactory::getApplication();
				$app->enqueueMessage(\Joomla\CMS\Language\Text::_('COM_JMEDIA_ERROR_WARNINVALID_FOLDER'));
			...
			if (!is_dir($path) && !is_file($path))
			{
				...
				{
					// There are some errors in the plugins
					JError::raiseWarning(100, \Joomla\CMS\Language\Text::plural('COM_JMEDIA_ERROR_BEFORE_SAVE', count($errors = $object_file->getErrors()), implode('<br />', $errors)));
				...
				if (JFolder::create($object_file->filepath))
				{
					...
					$this->setMessage(\Joomla\CMS\Language\Text::sprintf('COM_JMEDIA_CREATE_COMPLETE', substr($object_file->filepath, strlen(COM_JMEDIA_BASE))));
				}
			}
		...
		else
		{
			...
			JError::raiseWarning(100, \Joomla\CMS\Language\Text::_('COM_JMEDIA_ERROR_UNABLE_TO_CREATE_FOLDER_WARNDIRNAME'));
