...
class QuixModelPage extends JModelAdmin
{
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_quix.edit.page.data', []);
    ...
    public function save($data)
    {
        ...
        if ( ! isset($data['hits']) && ! isset($data['id']) ) {
            $data['hits'] = 0;
        }
        ...
        $data['modified'] = $data['modified'] ?? date('Y-m-d H:i:s');
    ...
    public function duplicate(&$pks)
    {
        ...
        if ( ! $user->authorise('core.create', 'com_quix.page')) {
            throw new Exception(\Joomla\CMS\Language\Text::_('JERROR_CORE_CREATE_NOT_PERMITTED'));
        }
    ...
    protected function canEditState($record)
    {
        ...
        return Factory::getUser()->authorise('core.edit.state', 'com_quix.page.'.(int) $record->id);
    }
    ...
    {
        if (empty($pks)) {
            $this->setError(\Joomla\CMS\Language\Text::_('COM_QUIX_NO_ITEM_SELECTED'));
