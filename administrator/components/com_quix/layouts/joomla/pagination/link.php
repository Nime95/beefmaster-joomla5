...
switch ((string) $item->text)
{
	...
	case \Joomla\CMS\Language\Text::_('JLIB_HTML_START') :
		$icon = 'icon-backward icon-first';
		break;

	// Check for "Prev" item
	case $item->text === \Joomla\CMS\Language\Text::_('JPREV') :
		$item->text = \Joomla\CMS\Language\Text::_('JPREVIOUS');
		$icon = 'icon-step-backward icon-previous';
		break;

	// Check for "Next" item
	case \Joomla\CMS\Language\Text::_('JNEXT') :
		$icon = 'icon-step-forward icon-next';
		break;

	// Check for "End" item
	case \Joomla\CMS\Language\Text::_('JLIB_HTML_END') :
