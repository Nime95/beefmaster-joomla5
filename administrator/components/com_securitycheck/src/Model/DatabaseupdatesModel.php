...
function add_vuln($array_complete,$local_database_version) {
	...
	foreach ($array_complete as $vulnerability) {
		/* Consultamos la rama para la que es válida la vulnerabilidad. Para ello dividimos los strings en el formato array[0]=3, array[1]=0... Así, el primer valor contendrá
		la rama para la que es válida la vulnerabilidad y la rama de joomla instalada */
		$vulnerabillity_branch = explode(".",$vulnerability['jversion']);
		$local_joomla_branch = explode(".",JVERSION);			
		
		// La versión de la vulnerabilidad debe ser mayor que la de la bbdd local para almacenarla
