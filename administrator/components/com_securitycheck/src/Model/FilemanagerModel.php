...
class FileManagerModel extends BaseDatabaseModel
{
...
function __construct()
{
	...
	$this->skipDirs[] = rtrim(Factory::getConfig()->get('tmp_path', JPATH_ROOT.'/tmp'), '/');
	$this->skipDirs[] = rtrim(Factory::getConfig()->get('log_path', JPATH_ROOT.'/logs'), '/');
...
public function getFiles($root = null)
{
	...
	$files_name = Folder::files($root,'.',true,true);
...
public function getDirectories($root = null)
{
	...
	$folders_name = Folder::folders($root,'.',true,true);
...
private function saveStack()
{
	...
	$old_files = Folder::files($this->folder_path,'.',false,true,array('index.html','web.config','.htaccess',$this->filemanager_name));
