...
if (!class_exists('QuixJoomlaMenuElement')) {
    class QuixJoomlaMenuElement
    {
        ...
        public static function prepareUrl(&$item)
        {
            ...
            if ($item->menu_image) {
                if ($item->menu_image_css) {
                    ...
                    $linktype                  = \Joomla\CMS\HTML\HTMLHelper::_('image', $item->menu_image, $item->title, $image_attributes);
                } else {
                    $linktype = \Joomla\CMS\HTML\HTMLHelper::_('image', $item->menu_image, $item->title);
                }
            ...
            return \Joomla\CMS\HTML\HTMLHelper::_('link', JFilterOutput::ampReplace(htmlspecialchars($item->flink, ENT_COMPAT, 'UTF-8', false)), $linktype, $attributes);
        }
        ...
        public static function getList(&$params)
        {
            ...
            $cache = Factory::getCache('quix-element-joomla-menu', '');
                ...
                if ($items) {
                    foreach ($items as $i => $item) {
                        ...
                        if ((strpos($item->flink, 'index.php?') !== false) && strcasecmp(substr($item->flink, 0, 4), 'http')) {
                            $item->flink = \Joomla\CMS\Router\Route::_($item->flink, true, $item->getParams()->get('secure'));
                        } else {
        ...
        public static function getBase(&$params)
        {
            ...
            if ($params->get('base')) {
                $base = Factory::getApplication()->getMenu('site')->getItem($params->get('base'));
            } else {
                $base = false;
            }
