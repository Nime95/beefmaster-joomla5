...
class QuixFormElementHelperEmail
{
    ...
    public static function action($data, $config, $info)
    {
        ...
        */
        ...
        $recipient = \Joomla\CMS\String\PunycodeHelper::emailToPunycode($recipient);
        ...
        // $email = \Joomla\CMS\String\PunycodeHelper::emailToPunycode($sysConfig->get('mailfrom'));
        ...
        if ($reply_to == 'system') {
            ...
            $replyTo = \Joomla\CMS\String\PunycodeHelper::emailToPunycode($data['email']);
        ...
        if ( ! empty($configNew['email_cc']->value)) {
            ...
            foreach ($emailCC as $key => $eCC) {
                $ccEmails[] = \Joomla\CMS\String\PunycodeHelper::emailToPunycode($eCC);
            }
            $mail->addCc($ccEmails);
        }
        if ( ! empty($configNew['email_bcc']->value)) {
            ...
            foreach ($email_bcc as $key2 => $emailBCC) {
                $bccEmails[] = \Joomla\CMS\String\PunycodeHelper::emailToPunycode($emailBCC);
            }
        ...
        if ($configNew['email_sendas']->value == 'html') {
            ...
            $body = strip_tags(\Joomla\CMS\Mail\MailHelper::cleanText($body));
            $mail->isHTML(false);
        }
