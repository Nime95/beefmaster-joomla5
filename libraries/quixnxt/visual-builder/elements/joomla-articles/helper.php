...
class QuixJoomlaArticlesElement
{
    ...
    public function getOptions()
    {
        ...
        if (!empty($extension)) {
            ...
            if ($published) {
                $options = \Joomla\CMS\HTML\HTMLHelper::_('category.options', $extension, ['filter.published' => explode(',', $published)]);
            } else {
                $options = \Joomla\CMS\HTML\HTMLHelper::_('category.options', $extension);
            }
            ...
            if (isset($this->element['show_root'])) {
                array_unshift($options, \Joomla\CMS\HTML\HTMLHelper::_('select.option', 'root', \Joomla\CMS\Language\Text::_('JGLOBAL_ROOT')));
            }
        } else {
            JLog::add(\Joomla\CMS\Language\Text::_('JLIB_FORM_ERROR_FIELDS_CATEGORY_ERROR_EXTENSION_EMPTY'), JLog::WARNING, 'jerror');
        }
    ...
    public static function getAjax($data = [])
    {
        ...
        $articles = \Joomla\CMS\MVC\Model\BaseDatabaseModel::getInstance('Articles', 'ContentModel', ['ignore_request' => true]);
        ...
        if ($catids) {
            if ($params->get('show_child_category_articles', 0) && (int) $params->get('category_depth', 999) > 0) {
                ...
                $categories = \Joomla\CMS\MVC\Model\BaseDatabaseModel::getInstance('Categories', 'ContentModel', ['ignore_request' => true]);
        ...
        foreach ($items as &$item) {
            ...
            if ($item->catid) {
                if (JVERSION < 4) {
                    ...
                    $item->displayCategoryLink = \Joomla\CMS\Router\Route::_(RouteHelper::getCategoryRoute($item->catid));
                }
    ...
    public static function truncate($html, $maxLength = 0)
    {
        ...
        $ptString = \Joomla\CMS\HTML\HTMLHelper::_('string.truncate', $html, $maxLength, $noSplit = true, $allowHtml = false);

        for ($maxLength; $maxLength < $baseLength;) {
            // Now get the string if we allow html.
            $htmlString = \Joomla\CMS\HTML\HTMLHelper::_('string.truncate', $html, $maxLength, $noSplit = true, $allowHtml = true);

            // Now get the plain text from the html string.
            $htmlStringToPtString = \Joomla\CMS\HTML\HTMLHelper::_('string.truncate', $htmlString, $maxLength, $noSplit = true, $allowHtml = false);
