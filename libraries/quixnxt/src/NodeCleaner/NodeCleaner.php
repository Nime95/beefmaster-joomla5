...
class NodeCleaner implements INodeCleaner
{
    ...
    public function merge(string $slug, array $data): array
    {
        ...
        if ( ! $defaults) {
            Factory::getDocument()->addScriptDeclaration("var qxAlerts = qxAlerts ?? [];qxAlerts.push('Sorry, Element is missing : <b>{$slug}</b>. If you save your page now, it will be lost forever.');");
            return [];
