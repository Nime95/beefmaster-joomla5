...
class TwigEngine extends RenderEngine
{
     ...
     * @since 4.1
     */
    public function hasAclJPermission(?int $acl = 1 ): bool
    {
        /**
         * first check ACL
    ...
    public function render(string $template, array $data = []): string
    {
        ...
        if ( ! $this->hasAclJPermission(intval($this->form['advanced']['identifier']['acl']))) {
            return '';
        }
    ...
    private function prepareTwig(): void
    {
        ...
        $this->twig->addFunction($this->isGdprComplianceEnabled());
        $this->twig->addFunction($this->JTranslate());
    ...
    private function isGdprComplianceEnabled(): TwigFunction
    {
      return new TwigFunction('isGdprComplianceEnabled', function () {
        return QUIX_GDPR_COMPLIANCE;
      });
    }

    private function JTranslate(): TwigFunction
    {
      return new TwigFunction('JTranslate', function ($text, $default = "") {
        $translated = Text::_($text);
        if($translated == $text) {
          $translated = $default;
        }
        return $translated;
    ...
    protected function getImageFunction(): TwigFunction
    {
        return new TwigFunction('image', function ($src, $alt = '', $cls = '', $attr = '', $source = [], $optimize = true) {
            ...
            if ( ! $optimize || !QUIXNXT_IMG_OPTIMIZER_ENABLE) {
                goto done;
            }
    ...
    protected function getLazyBackgroundFunction(): TwigFunction
    {
        return new TwigFunction('lazyBackground', function (?array $media, string $src = '') {
            ...
            if ($done || !QUIXNXT_IMG_OPTIMIZER_ENABLE) {
                goto done;
            }
    ...
    protected function getVideoFunction(): TwigFunction
    {
        return new TwigFunction('video', function ($id, $options, $attr = ' controls crossorigin playsinline settings ') {
            ...
            if ($linkType === 'youtube' || $linkType === 'vimeo') {
                ...
                $script  = \JUri::root(true) . '?quix-asset=assets/plyr.js&path=visual-builder&name=video';
                $style   = \JUri::root(true) . '?quix-asset=assets/plyr.css&path=visual-builder&name=video';
...
//                $style   = \QuixAppHelper::getQuixUrl().'/visual-builder/elements/video/assets/plyr.css';
        ...
        return new TwigFilter('link', function () {
            ...
            if ( ! preg_match('/^(https?:\/\/)|(http?:\/\/)|(\/\/)|([a-z\d-].)+(:[\d]+)(\/.*)?$/', $url)
                ...
                if (strpos($url, 'option=com_') !== false) {
                    $url = "href='".\Joomla\CMS\Router\Route::_($url)."'";
                } elseif ($url) {
                    $url = "href='".$url."'";
                }
            } elseif ($url) {
    ...
    protected function getPrepareContentFunction(): TwigFunction
    {
        return new TwigFunction('prepareContent', function ($text, $prepare = false) {
            return ($prepare ? \Joomla\CMS\HTML\HTMLHelper::_('content.prepare', $text) : $text);
        });
    }
    ...
    protected function getWrapperFunction(): TwigFunction
    {
        return new TwigFunction(
            ...
            function ($wrapper, $tag, $multipart = false, $end = false) {
                ...
                if ($tag === 'form') {
                    $url = \Joomla\CMS\Router\Route::_('index.php?option=com_quix');
    ...
    protected function getFormFooterFunction(): TwigFunction
    {
        ...
        return new TwigFunction('formFooter', function ($element, $config = []) use ($enc) {
			// get id from page from Joomla Application Input
            $app = \Joomla\CMS\Factory::getApplication();
			$id = $app->input->get('id');

			$filename = 'page-'.$id.'-quix-form-'.$config['advanced']['identifier']['id'].'.json';
	        try {
                // remove old file
		        if(file_exists(JPATH_SITE.'/cache/quixnxt/form/'.$filename)) {
			        \Joomla\Filesystem\File::delete( JPATH_SITE . '/cache/quixnxt/form/' . $filename );
		        }

	            // create new file
	            \Joomla\Filesystem\File::write(JPATH_SITE.'/cache/quixnxt/form/'.$filename, json_encode($config));
			...
			//'.$enc->encrypt(json_encode($config)).'
            return new Markup('
            ...
            <input type="hidden" name="jform[info]" value="'.$filename.'" />
            ', 'UTF-8');
        });
    }
    ...
    protected function getAddIconStyle(): TwigFunction
    {
        return new TwigFunction('addIconStyle', function ($selector, $src) {
            ...
            return new Markup("<style type=\"text/css\">$selector .playIcon img{width:{$sizeValue}px;height:{$sizeValue}px;} $selector i {color: $color;font-size: {$sizeValue}px;}$selector polygon,$selector path {fill: $color;}$selector svg {width: {$sizeValue}px;}</style>",
                'UTF-8');
        });
    }
}
