...
class QuixControllerPage extends JControllerForm
{
    ...
    public function cancel($key = 'id')
    {
        ...
        if (!$pk) {
            ...
            $link = \Joomla\CMS\Router\Route::_('index.php?option=com_quix&view=page&id=' . $pk);
    ...
    public function save($key = null, $urlVar = 'id')
    {
        ...
        if ($result) {
            ...
            // saved successfully
            $url = \Joomla\CMS\Router\Route::_('index.php?option=com_quix&view=form&layout=edit&tmpl=component&id=' . $id);
            $view = base64_encode('index.php?option=com_quix&view=page&preview=true&id=' . (int) $id);
            $edit = \Joomla\CMS\Router\Route::_('index.php?option=com_quix&view=form&layout=iframe&builder=frontend&id=' . $id);
    ...
    public function pageCreateAjax()
    {
        ...
        if ( !$model->save(['title' => $title, 'state' => 1, 'builder' => 'frontend', 'builder_version' => QUIXNXT_VERSION, 'data' => '[]'])) {
            echo new JResponseJson(New Exception(\Joomla\CMS\Language\Text::_('Failed to save')), '', 1, true);
        } else {
            $id = $model->getState('form.id');
            $link = JUri::root() . 'index.php?option=com_quix&task=page.edit&id='.$id;
            echo new JResponseJson($link, \Joomla\CMS\Language\Text::_('Page has been created.'), false, true);
        }
