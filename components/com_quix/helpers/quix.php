...
class QuixFrontendHelper
{
    ...
    public static function getModel($name)
    {
        ...
        if (file_exists(JPATH_SITE.'/components/com_quix/models/'.strtolower($name).'.php')) {
            ...
            $model = \Joomla\CMS\MVC\Model\BaseDatabaseModel::getInstance($name, 'QuixModel');
        }
    ...
    public static function getFlatIconsJSONfromServer()
    {
        ...
        try {
            ...
            if ($result->code != 200 && $result->code != 310) {
                $exception = new Exception(\Joomla\CMS\Language\Text::_('COM_QUIX_SERVER_RESPONSE_ERROR'));
    ...
    public static function saveOutputIconsJSON($data, $localHash = '', $storeHash = false)
    {
        ...
        if ($storeHash = false) {
            ...
            $cache->setCaching(\Factory::getApplication()->get('caching'));
        }
...
WRAP;
    ...
    {
        $array = [
            \Joomla\CMS\Language\Text::_('COM_QUIX_HINTS_TYPO'),
            \Joomla\CMS\Language\Text::_('COM_QUIX_HINTS_MY_ELEMENTS')
        ];
        $k     = array_rand($array);
    ...
    */
    ...
    public static function showPHPVersionWarning(){
        // Version warning
    ...
    public static function checkComponentAuth($app, $input){
        if ($input->get('view', '') === 'form')
        {
            ...
            if (!$hasAccess)
            {
                $app->enqueueMessage(\Joomla\CMS\Language\Text::_('JERROR_ALERTNOAUTHOR'), 'warning');
                if ($user->get('guest'))
                {
