...
class QuixFrontendHelperAMP
{
    ...
    {
        \Joomla\CMS\MVC\Model\BaseDatabaseModel::addIncludePath(JPATH_SITE . '/administrator/components/com_menus/models');

        $app = Factory::getApplication();
        $results = [];

        if ($menutype) {
            // $model = \Joomla\CMS\MVC\Model\BaseDatabaseModel::getInstance('Items', 'MenusModel', ['ignore_request' => true]);
            // $model->getState();
            ...
            for ($i = 0, $n = count($results); $i < $n; $i++) {
                ...
                switch ($results[$i]->type) {
                    ...
                    case 'alias':
                        ...
                        if (JLanguageMultilang::isEnabled()) {
                            $newItem = Factory::getApplication()->getMenu()->getItem((int) $results[$i]->params->get('aliasoptions'));
                ...
                if ((strpos($results[$i]->flink, 'index.php?') !== false) && strcasecmp(substr($results[$i]->flink, 0, 4), 'http')) {
                    $results[$i]->flink = \Joomla\CMS\Router\Route::_($results[$i]->flink, true, $results[$i]->params->get('secure'));
                } else {
                    $results[$i]->flink = \Joomla\CMS\Router\Route::_($results[$i]->flink);
                }
            }
        }
