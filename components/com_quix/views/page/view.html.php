...
class QuixViewPage extends HtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        if ( ! isset($this->item->id) || ! $this->item->id) {
            $this->app->enqueueMessage(\Joomla\CMS\Language\Text::_('JERROR_PAGE_NOT_FOUND'), 'error');
            $this->app->setHeader('status', 404, true);

            $error = new Exception(\Joomla\CMS\Language\Text::_('JERROR_PAGE_NOT_FOUND'), 404);
        ...
        if ($this->item->params->get('access-view') === false
            ...
            $error = new Exception(\Joomla\CMS\Language\Text::_('JERROR_ALERTNOAUTHOR'), 403);
            return ExceptionHandler::render($error);
        }
...
//        }
        ...
        if ($this->app->get('sitename_pagetitles', 0) === '1') {
            $title = \Joomla\CMS\Language\Text::sprintf('JPAGETITLE', $this->app->get('sitename'), $title);
        } elseif ($this->app->get('sitename_pagetitles', 0) === '2') {
            $title = \Joomla\CMS\Language\Text::sprintf('JPAGETITLE', $title, $this->app->get('sitename'));
        } elseif (empty($title)) {
            $title = $this->params->get('page_title', \Joomla\CMS\Language\Text::_('COM_QUIX_DEFAULT_PAGE_TITLE'));
        }
