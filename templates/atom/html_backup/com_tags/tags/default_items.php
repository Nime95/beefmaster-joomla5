...
// \Joomla\CMS\HTML\HTMLHelper::_('behavior.caption');
...
<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		...
		<fieldset class="filters btn-toolbar">
			<?php if ($this->params->get('filter_field')) : ?>
				<div class="qx-button-group">
					<input type="text" name="filter-search" id="filter-search" value="<?php echo $this->escape($this->state->get('list.filter')); ?>" class="qx-input" onchange="document.adminForm.submit();" title="<?php echo \Joomla\CMS\Language\Text::_('COM_TAGS_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo \Joomla\CMS\Language\Text::_('COM_TAGS_TITLE_FILTER_LABEL'); ?>" />
	...
	<?php if ($this->items == false || $n === 0) : ?>
		<p><?php echo \Joomla\CMS\Language\Text::_('COM_TAGS_NO_TAGS'); ?></p>
	<?php else : ?>
		<?php foreach ($this->items as $i => $item) : ?>
			...
			<?php if ((!empty($item->access)) && in_array($item->access, $this->user->getAuthorisedViewLevels())) : ?>
				<li class="cat-list-row<?php echo $i % 2; ?>">
					<h3>
						<a href="<?php echo \Joomla\CMS\Router\Route::_(TagsHelperRoute::getTagRoute($item->id . ':' . $item->alias)); ?>" class="qx-text-large qx-text-bold">
			...
			<?php if (($this->params->get('all_tags_show_tag_description', 1) && !empty($item->description)) || $this->params->get('all_tags_show_tag_hits')) : ?>
				<div class="caption">
					<?php if ($this->params->get('all_tags_show_tag_description', 1) && !empty($item->description)) : ?>
						<span class="tag-body">
							<?php echo \Joomla\CMS\HTML\HTMLHelper::_('string.truncate', $item->description, $this->params->get('all_tags_tag_maximum_characters')); ?>
						</span>
					<?php endif; ?>
					<?php if ($this->params->get('all_tags_show_tag_hits')) : ?>
						<span class="list-hits badge badge-info">
							<?php echo \Joomla\CMS\Language\Text::sprintf('JGLOBAL_HITS_COUNT', $item->hits); ?>
						</span>
					<?php endif; ?>
				</div>
			<?php endif; ?>
