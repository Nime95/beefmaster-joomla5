...
class JFormFieldTypography extends FormField
{
      ...
      protected function getInput() {
            ...
            $template_path = JPATH_SITE . '/templates/' . self::getTemplate() . '/admin/webfonts/webfonts.json';
            ...
            $html .= '<label><strong>'. JText::_('Font Family') .'</strong></label>';
            ...
            $html .= '<label><strong>'. JText::_('Font Weight & Style') .'</strong></label>';
            ...
            $html .= '<label><strong>'. JText::_('Font Subset') .'</strong></label>';
            ...
            $html .= '<label><strong>'. JText::_('Font Size') .'</strong></label>';
      ...
      private static function getTemplate() {
            ...
            $styleId = $input->get('id', 0, 'INT');        
            
            $db = JFactory::getDbo();
            $query = $db->getQuery(true);
            $query->select($db->quoteName(array('template')));
            $query->from($db->quoteName('#__template_styles'));
            $query->where($db->quoteName('id') . ' = ' . (int) $styleId);
