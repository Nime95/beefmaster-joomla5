...
class PlgSystemSeositeattributes extends CMSPlugin
{
	/**
	...
	public function onBeforeRender()
	{
		...
		if (Factory::getApplication()->isClient('administrator'))
		{
			...
			if('com_quix' == $option && ('pages' == $view or 'collections' == $view or 'integrations' == $view or 'dashboard' == $view or 'elements' == $view or 'filemanager' == $view or 'help' == $view)){
				...
				$link = \Joomla\CMS\Router\Route::_("index.php?option=com_plugins&client_id=0&task=plugin.edit&extension_id=".$pluginid);

				$toolbar = \Joomla\CMS\Toolbar\Toolbar::getInstance('toolbar');
				$toolbar->appendButton('Custom', "<a href='".$link ."' target='_blank' class='btn hasPopover' data-title='".\Joomla\CMS\Language\Text::_('PLG_SYSTEM_SEOSITEATTRIBUTES')."' data-content='".\Joomla\CMS\Language\Text::_('PLG_SYSTEM_SEOSITEATTRIBUTES_INTRO')."' data-placement='bottom'>".\Joomla\CMS\Language\Text::_('PLG_SYSTEM_SEOSITEATTRIBUTES_TITLE')."</a>");
			}
			return;
		}
		...
		$name = $this->params->get('name', Factory::getConfig()->get('sitename'));
		...
		Factory::getDocument()->addScriptDeclaration(json_encode($structured_markup), 'application/ld+json');
